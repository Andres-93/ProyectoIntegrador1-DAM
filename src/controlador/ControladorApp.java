...
public class ControladorApp implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		// Si entramos como profesores tendremos los permisos, en caso de introducir
		// usuario o contraseña no validos nos lo muestra por un Joption.
		if (e.getSource().equals(panel2.getBtnBoton())) {
			if (new ProfesorPersistencia().buscarUsuario(panel2.getDatos())) {
				...
				JOptionPane.showConfirmDialog(null, "Usuario o contraseña no validos, vuelva a intentarlo",
						"Error al logearse", JOptionPane.CLOSED_OPTION);
			}
			...
			int respuesta = JOptionPane.showConfirmDialog(panel2,
					"Si entra con una cuenta de Alumno unicamente podra realizar consultas. Desea Continuar?",
			...
			vistaP.setPanel(panel3);
				...
				|| e.getSource().equals(panel5.getButton()) || e.getSource().equals(panel7.getBtnCancelCicloA())
				|| e.getSource().equals(panel9.getBtnCanceloar()) || e.getSource().equals(panel10.getBtnCancelar())
				|| e.getSource().equals(panel12.getBtnCancelar()) || e.getSource().equals(panel14.getBtnCancelar())
				|| e.getSource().equals(panel15.getBtnCancelar()) || e.getSource().equals(panel17.getBtnCancelar())
				|| e.getSource().equals(panel18.getBtnCancelar())) {
			vistaP.setPanel(panel1);
			...
			if (panel6.getDatos().getUsuario().equals("") || panel6.getDatos().getPass().equals("")) { // POR ALGUN
																										// MOTIVOS, AUN
																										// SIENDO LOS
																										// CAMPOS NOT
																										// NULL PERMITE
																										// GUARDAR DATOS
																										// NULL
				JOptionPane.showConfirmDialog(null, "Ninguno de los campos debe estar en blanco", "Error",
			...
			if (panel7.getDatos().getNombre().equals("")) {
				JOptionPane.showConfirmDialog(null, "No puede dejar el nombre en blanco", "Error",
						JOptionPane.CLOSED_OPTION);
			} else {
			...
			new AlumnoPersistencia().modificarAlumno(panel8.getDatos(),
					new AlumnoPersistencia().obtenerID(panel5.obtenerSeleccionado()));
			panel8.setVisible(false);
			panel5.mostrarAlumnos(new AlumnoPersistencia().cargarAlumnos()); // Tras modificar el usuario volveremos a
																				// cargar la lista actualizada de
																				// alumns.

		} else if (e.getSource().equals(panel9.getBtnEliminarCiclo())) {
			if(panel9.ciclossAEliminar() != null) {
				new CicloPersistencia().eliminarCiclo(panel9.ciclossAEliminar());
				panel9.mostrarCiclos(new CicloPersistencia().cargarCiclos());
			}else {
				JOptionPane.showMessageDialog(null, "Debe seleccionar un ciclo para eliminarlo");
			}
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "Debe seleccionar un ciclo para modificarle");
			}
			...
			new CicloPersistencia().modificarAlumno(panel11.getDatos(),
					new CicloPersistencia().obtenerID(panel10.cicloSelecionado().getNombre()));
			panel11.setVisible(false);
			panel10.mostrarCiclos(new CicloPersistencia().cargarCiclos());
			...
			if (!panel13.getDatos().getNombre().equals("")) {
				new PoryectoPersistencia().añadirProyecto(panel13.getDatos());
			} else {
				JOptionPane.showMessageDialog(null, "Porfavor no deje el nombre del proyecto en blanco");
			}
			...
			if (panel14.ProyectoAEliminar() != null) {
				new PoryectoPersistencia().eliminarProyecto(panel14.ProyectoAEliminar());
			} else {
				JOptionPane.showMessageDialog(null, "Debe seleccionar un proyecto para eliminarlo");
			}
			...
			if (panel15.obtenerSelecionado() != null) {
				panel16.mostrarDatos(panel15.obtenerSelecionado());
				panel16.setVisible(true);
			} else {
				JOptionPane.showMessageDialog(null, "Debe seleccionar un proyecto para modificarle");
			}
			...
			new PoryectoPersistencia().vincularAlumno(panel16.añadirAlumno(),
					new PoryectoPersistencia().obtenerID(panel15.obtenerSelecionado())); // Pruebas
			panel16.mostrarDatos(panel15.obtenerSelecionado());

		} else if (e.getSource().equals(panel16.getBtnElimAl())) {
			new PoryectoPersistencia().desvincularAlumno(panel16.eliminarAlumno(),
					new PoryectoPersistencia().obtenerID(panel15.obtenerSelecionado()));
			panel16.mostrarDatos(panel15.obtenerSelecionado());

		} else if (e.getSource().equals(panel16.getBtnModificarProJD())) {
			new PoryectoPersistencia().modificarProyecto(panel16.getDatos(),
					new PoryectoPersistencia().obtenerID(panel15.obtenerSelecionado()),
					new CicloPersistencia().obtenerID(panel16.obtenerNombreCiclo()));
			panel16.dispose();
			panel15.mostrarProyectos(new PoryectoPersistencia().cargarProyectos());
			...
			if (panel18.proyectoSeleccionado() != null) {
				panel19.mostrarProyecto(panel18.proyectoSeleccionado());
				panel19.setVisible(true);
			} else {
				JOptionPane.showMessageDialog(null, "Debe seleccionar un proyecto para verle en detalle");
			}
