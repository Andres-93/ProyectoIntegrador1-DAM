...
public class ProfesorPersistencia {
	...
	public boolean buscarUsuario(Profesor profe) {
		...
		try {
			...
			st.setString(1, profe.getUsuario());
			st.setString(2, profe.getPass());
			// Si queremos guardar todas las columnas en una especie de coleccion haremos
			// ResultSet rs = st.executeQuery();

			rs = st.executeQuery();

			while (rs.next()) {
				if (rs.getString("usuario").equals(profe.getUsuario())
						&& rs.getString("contraseña").equals(profe.getPass())) {
					encontrado = true;
				}
			}
		} catch (SQLException e) {
